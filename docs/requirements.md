...
収支処理は現金を中心に行い、年末に資産間のリバランスを行う。株安時は株式の売却を避け、現金・国債を有効活用する設計。
...
| 6 | 現金補填 | 現金が上限未満の場合、下落状況に応じて補填 |

#### 現金補填ロジック（ステップ6）

年末に現金が上限未満の場合、資産の下落率に応じて補填元を決定する。

| 条件 | 補填元 | 理由 |
|------|--------|------|
| 下落率が0%以下（資産減少または変化なし） | 国債優先 | 株安時は株式を温存 |
| 下落率が0%超（資産増加） | 株式優先 | 株高時は株式から利益確定 |
...
各シミュレーション試行（1,000回）:
  ...
  各年のループ:
    ...
    5. 現金補填（下落状況に応じて）
       下落率 = (現在総資産 - 前年総資産) / 前年総資産 × 100

       if 現金残高 < 現金上限:
         不足額 = 現金上限 - 現金残高

         if 下落率 ≦ 0:  // 資産減少または変化なし
           // 国債から優先補填（株式温存）
           国債から補填 → 不足なら株式から補填
         else:
           // 株式から優先補填（利益確定）
...
| 処理 | 株安時の挙動 | 効果 |
|------|-------------|------|
| 収支処理（ステップ2） | 現金不足時は国債→株式の順で補填 | 株式は最後の手段 |
| 現金補填（ステップ5） | 下落率が0%以下なら国債から補填 | 株式を温存 |
...
| 項目 | 影響 | 対応の難易度 |
|------|------|-------------|
| 手数料未考慮 | 信託報酬等により実際のリターンはやや低くなる | 低 |
| 資産間の相関未考慮 | 暴落時に株式・国債が同時に下落するリスクを過小評価 | 高 |
| 為替リスク未考慮 | 海外資産の場合、為替変動の影響がある | 高 |
| 行動バイアス未考慮 | 暴落時に計画通り行動できない可能性 | 対応不可（人間の問題） |
| 単一の資産クラス | 不動産、金、暗号資産などは対象外 | 中 |
