...
| 項目 | 説明 | 単位 | デフォルト値 |
|------|------|------|-------------|
| 株式・投資信託 | ファンド別に登録（下記参照） | — | 空（ファンド未登録） |
| 国債 | 現在の国債保有額 | 万円 | 0 |
...
株式・投資信託はファンド単位でテーブル形式で入力する。各ファンドにはプリセットが用意されており、選択すると外貨比率が自動設定される（手動編集も可能）。最大10ファンドまで登録可能。

**ファンドのデータ構造:**

| フィールド | 説明 |
|-----------|------|
| プリセット | ファンド種類の選択（下記プリセット一覧） |
| 金額（万円） | ファンドの保有額 |
| 外貨比率（%） | 外貨建て資産の割合（プリセットから自動設定、手動編集可） |

**プリセット一覧:**

| ID | 表示名 | 外貨比率 | 根拠 |
|----|--------|---------|------|
| `all_country` | 全世界株式（オルカン） | 88% | MSCI ACWI 日本比率 ~5.5%、残りが海外 |
| `developed_ex_jp` | 先進国株式（MSCIコクサイ） | 100% | 日本除く先進国で構成 |
| `sp500` | S&P 500 / 米国株式 | 100% | 全額USD建て |
| `domestic` | 国内株式 | 0% | 全額JPY建て |
| `emerging` | 新興国株式 | 100% | 全額外貨建て |
| `balanced` | バランス型（8資産均等） | 75% | 国内株式・債券25%、海外75%の典型的構成 |

※外貨比率の値は概算値。厳密な最新値はファンドにより異なるため、プリセット選択後でも手動で編集可能。

**集約関数（シミュレーション接続）:**

シミュレーション実行時、株式ファンドの情報は以下の集約関数で合計額と加重平均外貨比率に変換され、既存のシミュレーションエンジンにそのまま渡される。

- `getTotalStocks(funds)`: 全ファンドの金額合計
- `getWeightedForeignRatio(funds)`: 金額による加重平均外貨比率（全ファンド金額0の場合は0%）

**外貨比率の役割**: 加重平均外貨比率に基づき、外貨部分にはレジームに連動した為替変動が適用され、円建ての実質リターンがシミュレーションに反映される。詳細は3.3.6を参照。
...
旧形式（`stocks` + `foreignRatio`）のデータを読み込んだ際、新形式（`stockFunds[]`）に自動変換する。変換はクライアントサイドの読み込み時に行い、Supabase・localStorageの両方に適用される。

| 旧 foreignRatio | 選択されるプリセット | 理由 |
|-----------------|-------------------|------|
| 0% | 国内株式 | 外貨なし |
| 85-92% | 全世界株式（オルカン） | オルカンの典型的な外貨比率 |
| 100% | S&P 500 / 米国株式 | 全額外貨 |
| その他 | バランス型（8資産均等） | 既知のプリセットに合致しない |

※`foreignRatio` の値自体は変換後もそのまま維持するため、プリセット選択はラベル表示のための参考に過ぎない。
...
- 株式ファンド一覧（各ファンド: プリセットID、表示名、金額、外貨比率）
- 国債保有額
- 国債積立上限
- 現金保有額
- 現金積立上限
...
| 2.23 | 2026/02/21 | 株式ファンド入力UIを追加：単一の株式額＋外貨比率からファンド別テーブル入力に変更、プリセット付き（全世界株式、先進国株式、S&P500、国内株式、新興国株式、バランス型）、外貨比率は加重平均で自動算出、旧データからの自動マイグレーション対応 |
