...
| 税金考慮 | 株式・国債のリターンに対して税率を適用。税率（デフォルト20%）を設定し、リターン×(1-税率)で税引後リターンを計算。NISAのみの場合は0%に設定。利益部分にのみ課税される仕組みを反映 |
...
| 項目 | 影響 | 対応の難易度 |
|------|------|-------------|
| 税金・手数料未考慮 | 実際のリターンは1-2%程度低くなる | 中（税制が複雑） |
| 資産間の相関未考慮 | 暴落時に株式・国債が同時に下落するリスクを過小評価 | 高 |
| 年次単位のシミュレーション | 年内の急落・急騰は無視される | 低（月次にすれば可能） |
...
| 項目 | 影響度 | コメント |
|------|--------|----------|
| 税金・手数料 | 大 | NISAなら非課税、特定口座なら約20%の税金 |
| 資産間の相関 | 中 | 国債比率が低ければ影響は限定的 |
| 年次単位 | 小 | 長期シミュレーションでは大きな問題にならない |
