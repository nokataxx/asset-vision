# 改善Todo リスト

作成日: 2026-01-29

---

## 優先度: 高

### コード品質

- [x] **ESLintエラー修正: button.tsx** ✅ 2026-01-29
  - 箇所: `src/components/ui/button.tsx:62`
  - 対応: `buttonVariants`のエクスポートを削除（内部でのみ使用のため）

- [x] **ESLintエラー修正: regime.ts** ✅ 2026-01-29
  - 箇所: `src/lib/simulation/regime.ts:134`
  - 対応: 未使用パラメータ`_settings`を削除、呼び出し元も修正

### 入力バリデーション

- [ ] **資産入力のバリデーション追加**
  - 資産額: 0以上、上限10億円
  - 株式・国債・現金それぞれに適用

- [ ] **収支計画のバリデーション追加**
  - 成長率: -10%〜+10%の範囲制限
  - シミュレーション期間: 1〜60年に制限

- [ ] **バリデーションライブラリ導入検討**
  - Zodなどのスキーマバリデーションで型安全な検証

### シミュレーションロジック修正

- [ ] **回復目標のエッジケース修正**
  - 箇所: `src/lib/simulation/regime.ts:110`
  - 現状: `precrashStocksBalance += netIncome`
  - 修正: `precrashStocksBalance = Math.max(0, precrashStocksBalance + netIncome)`
  - 理由: 継続的な赤字で回復目標が負になる問題を防止

---

## 優先度: 中

### エラーハンドリング

- [ ] **Supabaseエラーのユーザー通知**
  - 箇所: `src/lib/database/userDataService.ts`
  - 現状: console.logのみ
  - 改善: トースト通知でエラーを表示

- [ ] **オフライン状態の検出と表示**
  - ネットワーク切断時にユーザーに通知

- [ ] **localStorage容量超過の警告**
  - 60年以上の長期シミュレーションで容量超過の可能性

### 型安全性強化

- [ ] **データ読み込み時の型検証**
  - 箇所: `src/lib/database/userDataService.ts:48`
  - 現状: `const row = data as UserSimulationDataRow`（検証なしキャスト）
  - 改善: Zodでスキーマ検証を実施

- [ ] **マイグレーションロジックの型ガード強化**
  - 箇所: `src/App.tsx:64-70`
  - 旧フォーマットからの移行処理に型安全性を追加

### 計算ロジック改善

- [ ] **安全引出率（SWR）計算の改善**
  - 箇所: `src/lib/simulation/statistics.ts:245-246`
  - 現状: 初年度の引出額のみで計算
  - 改善: 期間平均または最大引出年の値を使用

- [ ] **50%タイル試行の選択方法改善**
  - 現状: 二乗誤差距離が最小の試行を選択
  - 改善案1: 「代表的試行」と明記
  - 改善案2: 複数指標（資産推移＋暴落回数）で選択

---

## 優先度: 低

### パフォーマンス

- [ ] **モンテカルロシミュレーションの並列化**
  - 現状: 1,000回試行が直列実行（UIブロック100ms+）
  - 改善: Web Workerで並列化
  - 期待効果: 4-8倍の高速化

- [ ] **年次テーブルの仮想化**
  - 現状: 60年以上でDOM性能低下
  - 改善: react-virtualなど導入

- [ ] **バンドルサイズ削減**
  - 現状: 812KB (gzip 239KB)
  - 改善: コード分割、Rechartsの部分インポート
  - 目標: gzip 180KB以下

### 再現性

- [ ] **シード付き乱数生成器の導入**
  - 現状: `Math.random()`使用、シードなし
  - 改善: seedrandomなど導入
  - 効果: シミュレーション結果の再現・検証が可能に

### 自動保存

- [ ] **「今すぐ保存」ボタン追加**
  - 現状: 2秒遅延の自動保存のみ
  - 問題: タブ即閉じでデータ喪失の可能性

- [ ] **保存状態インジケーター追加**
  - クラウドアイコンで保存状態を表示

### ドキュメント

- [ ] **シミュレーションロジックへのコメント追加**
  - `regime.ts`: レジーム遷移ロジックにJSDocコメント
  - `monteCarlo.ts`: 年次処理フローの説明コメント

- [ ] **ARCHITECTURE.md作成（任意）**
  - システム全体のアーキテクチャ説明

### テスト

- [ ] **テストスイート追加**
  - Jest + React Testing Library導入
  - 対象:
    - 回復目標のエッジケース（負の目標値）
    - 0年シミュレーション
    - 初期資産0で支出ありのケース
    - レジーム遷移確率
    - パーセンタイル計算の精度

---

## 枯渇判定の不整合（調査必要）

- [ ] **枯渇判定と表示の整合性確認**
  - `isDepleted()`: `totalAssets <= 0`でチェック
  - 表示: `Math.max(0, balance)`でクランプ
  - 問題: 非枯渇でも資産0表示の可能性
  - 対応: 一貫した丸め処理の適用

---

## 完了項目

（完了したらここに移動）

